...
FMS	.BYTE	0
	...
	.WORD	INIT
;-------------------------------------------------------------------
;LOADER  $F00 (WRITE BUFFER)
;-------------------------------------------------------------------
;	BOOT ROUTINE
;-------------------------------------------------------------------
	LDX	#WMES-$280&$FF	;SEND OUT WELCOME MESSAGE
	LDY	#WMES-$280/256	;$780 -> $E00
...
;-------------------------------------------------------------------
WMES	.BYTE	"LiteDOS-XS (c) Mr.Atari 2019",$9B
;-------------------------------------------------------------------
...
REDO	JSR	REAVTO		;GET DOSTYPE, TRIP ON DD-DISK
	...
	CMP	#%00001010	;CHECK WRITE/UPDATE
...
DPB	STA	DDATA		;STORE
	...
	AND	#$08
	BEQ	FNO
	CPX	RBUFFER		;CHECK IF VALID IOCB
	BNE	FNO
	JMP	DPB1
...
;-------------------------------------------------------------------
;	FAST CIO, READ USES IOCB #1
;-------------------------------------------------------------------

;	CLOSE READ/DIR

FCIOC	JSR	FCIOCR		;A=$FF, Y=1
...
;-------------------------------------------------------------------
WRIBUF	LDX	#WRITE		;#PUTSEC/#WRITE PUT/WRITE
...
;-------------------------------------------------------------------
ODO	LDY	#176		;WRONG DOS-ERROR
	LDA	DRBUF		;CHECK DOSTYPE
	CMP	#$83
	BNE	ODOERR

	LDY	#32		;COPY VTOC TO VTOBUF
...
WRIVTOC	JSR	REAVTO		;GET SECTOR, DON'T USE THE DATA
	...
	LDX	#31		;COPY VTOBUF TO DSKBUF, COUNT BITS
DSKVTO	LDA	VTOBUF,X
	STA	DWBUF+32,X
	LDY	#8		;COUNT BITS
WVTSH	LSR	A
...
ALLO2	LDA	VTOBUF,X	;READ BITMAP
	...
	INX
	CPX	#32		;MAXIMUM 64 VTOC BYTES TO CHECK
...
ALLO5	TXA			;0-31 WITH CARRY (0-63)
	...
	ASL	FMSNE1		;CALCULATE START SECTOR OF THIS CLUSTER
	ROL	FMSNE2
	ASL	FMSNE1
	ROL	FMSNE2		;XX-XXXYYY00

ALLODON	
;-------------------------------------------------------------------
IMPSEN	LDA	DWBUF+127-1	;IO-SECTOR FROM BUFFER TO DAUX
	STA	DAUX1
	LDA	DWBUF+127-2
	AND	#$03		;FF OR 03
	...
	STA	DWBUF+127-1
	LDA	FMSNE2		;ALWAYS 2 BIT...
	STA	DWBUF+127-2

	LDA	DWPTR		;BYTES
	STA	DWBUF+127	;PUT SIZE IN SECTOR-BUFFER
...
TOGGLE	LDA	DAUX1		;CHECK FIRST SECTOR IN CLUSTER
	...
	LDA	#$03		;GET CLUSTER SIZE
TOSHIFT	LSR	DAUX2		;CALCULATE CLUSTER OF THIS SECTOR
	ROR	DAUX1
	LSR	A
	BNE	TOSHIFT		;00000000-XXXXXYYY IN DAUX
